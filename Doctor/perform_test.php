...
<body>
            ...
            <div class="container-fluid">
                <div class="row">
                    ...
                    <div class="col-lg-12" style="display: flex; justify-content: space-between; align-items: center;padding-top:20px;padding-bottom:20px">
                        <h1 class="page-header" style="color: red; margin: 0; margin-left: auto; margin-right: auto;">MEDICAL EXAMINATION</h1>
                        ...
                        <div class="panel panel-default">
                            ...
                            <div class="panel-heading">
                                ...
                                if ($row = mysqli_fetch_array($result)) {
                                    ...
                                    echo "<span style='font-size: larger; color: blue;'>اسم المتبرع : </span><span style='font-size: larger; color: blue;'>" . $row['name'] . " </span><span style='font-size: larger; color: blue;'>" . $row['name_family'] . "</span>";
                                    echo "<span style='display: inline-block; width: 20px;'></span>";  // Adding space between the elements
                                    echo "<span style='font-size: larger; color: blue;'>تاريخ الميلاد : </span><span style='font-size: larger; color: blue;'>" . $row['dob'] . " </span>";
                                    echo "</div>";
                                    echo "</div>";
                                } else {
                                    echo "<span style='font-size: larger; color: blue;'> لم يتم العثور على سجل </span>";
                                }
                                ?> </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // إضافة مستمع للتحقق من النطاقات الخاصة بالإدخالات
            document.querySelectorAll('input[type="number"]').forEach(function(input) {
                input.addEventListener('change', function() {
                    var value = parseFloat(input.value);
                    if (isNaN(value)) return; // الخروج إذا كانت القيمة ليست رقمًا
                    var min = parseFloat(input.getAttribute('min'));
                    var max = parseFloat(input.getAttribute('max'));
                    if (value < min || value > max) {
                        input.classList.add('out-of-range'); // إظهار إشارة الخطأ
                    } else {
                        input.classList.remove('out-of-range'); // إزالة إشارة الخطأ
                    }
                });
            });
        });
