...
$d = date('Y-m-d');
$query1         = mysqli_query($conn, "SELECT * FROM Blood_Separation WHERE Separation_Date = '$d' ");
$num_row1     = mysqli_num_rows($query1);

$query2         = mysqli_query($conn, "SELECT * FROM Blood_Separation WHERE Separation_Date = '$d' and Unit_Type = 'RPC' ");
$num_row2     = mysqli_num_rows($query2);

$query3         = mysqli_query($conn, "SELECT * FROM Blood_Separation WHERE Separation_Date = '$d' and Unit_Type = 'FFP' ");
$num_row3     = mysqli_num_rows($query3);

$query4         = mysqli_query($conn, "SELECT * FROM Blood_Separation WHERE Separation_Date = '$d' and Unit_Type = 'PU'  ");
$num_row4     = mysqli_num_rows($query4);

$query5         = mysqli_query($conn, "SELECT * FROM Blood_Separation WHERE Separation_Date = '$d' and Unit_Type = 'WB' ");
...
<body>
    ...
    <script>
        ...
        var myChart = new Chart(ctx, {
            ...
            data: {
                ...
                labels: ['Separations Total', 'Separations Today', 'Separations RBC']
            },
            options: {
                title: {
                   display: true,
    ...
    <script>
        ...
        var myChart = new Chart(ctx, {
            ...
            data: {
                datasets: [{
                    data: [<?php echo $num_row2 ; ?>, <?php echo $num_row3  ; ?>, <?php echo $num_row4; ?>, <?php echo $num_row5 ; ?>],
                    backgroundColor: ['rgba(205, 9.9, 1.32, 105)', 'rgba(54, 16.2, 235, 0.5)', 'rgba(2.55, 206, 86, 0.5)', 'rgba(255, 206, 86, 0.5)'],
                }],
                labels: ['Separations RBC', 'Separations FFP', 'Separations PU', 'Separations WB']
            },
            options: {
                title: {
                   display: true,
